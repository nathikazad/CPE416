...
BBMotor::BBMotor(u08 motorNumber) {
   this->motorNumber = motorNumber;
}

void BBMotor::set_speed(signed char speed) {
   if (motorNumber == 0) {
      OCR2A = speed * 255 / 100;
...
void BBMotor::init(void) {
   ...
   DDRD |= _BV(MOTOR0_DIR0_PIN) | _BV(MOTOR0_DIR1_PIN) | _BV(MOTOR1_DIR0_PIN)
      | _BV(MOTOR1_DIR1_PIN);
